...
#include "Button_1.h"
    ...
    #define Button_1_SetP4PinDriveMode(shift, mode)  \
    do { \
        Button_1_PC =   (Button_1_PC & \
                                (uint32)(~(uint32)(Button_1_DRIVE_MODE_IND_MASK << \
                                (Button_1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Button_1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Button_1_SetP4PinDriveMode(shift, mode)  \
        do { \
            Button_1_USBIO_CTRL_REG = (Button_1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Button_1_DRIVE_MODE_IND_MASK << \
                                    (Button_1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Button_1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Button_1_SetP4PinDriveMode(Button_1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Button_1_DR & (uint8)(~Button_1_MASK));
    drVal = (drVal | ((uint8)(value << Button_1_SHIFT) & Button_1_MASK));
    Button_1_DR = (uint32)drVal;
}
...
{
    return (uint8)((Button_1_PS & Button_1_MASK) >> Button_1_SHIFT);
}
...
{
    return (uint8)((Button_1_DR & Button_1_MASK) >> Button_1_SHIFT);
}
...
void Button_1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Button_1_INTCFG & (uint32)(~(uint32)position);
    Button_1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Button_1_INTSTAT & Button_1_MASK);
	Button_1_INTSTAT = maskedStatus;
    return maskedStatus >> Button_1_SHIFT;
}
